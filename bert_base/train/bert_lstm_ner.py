...
class NerProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "train_words.txt"),os.path.join(data_dir, "train_tags.txt")), "train"
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "test_words.txt"),os.path.join(data_dir, "test_tags.txt")), "dev"
        )

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "test_words.txt"),os.path.join(data_dir, "test_tags.txt")), "test")
    ...
    def get_labels(self, labels=None):
        ...
        self.labels= ["O","B_time","I_time","E_time","B_dname","I_dname","E_dname","B_hname","I_hname","E_hname","[CLS]","[SEP]"]
        return self.labels
    ...
    def _read_data(self, words_file,tags_file):
        """Reads a BIO data."""
        # todo  中文文件 预处理
        f_words = open(words_file,"r",encoding ="utf-8")
        f_tags = open(tags_file,"r",encoding="utf-8")
        words_lst = f_words.readlines()
        tags_lst  = f_tags.readlines()
        words_lst = [ item.replace("\n","") for item in words_lst]
        tags_lst = [ item.replace("\n","") for item in tags_lst]
        assert  (len(words_lst) == len (tags_lst))

        lines = [(tag,word) for tag,word in zip (tags_lst,words_lst)]
        return lines
        #####
        # with codecs.open(input_file, 'r', encoding='utf-8') as f:
        #     lines = []
